...
export function HeroSection() {
  ...
  const [typingSpeed, setTypingSpeed] = useState(120); // Slightly adjusted speed
  const [charIndex, setCharIndex] = useState(0);

  useEffect(() => {
    const currentFullHeadline = headlines[currentHeadlineIndex];

    const handleTyping = () => {
      if (isDeleting) {
        if (charIndex > 0) {
          setDisplayedHeadline(currentFullHeadline.substring(0, charIndex - 1));
          setCharIndex(charIndex - 1);
          setTypingSpeed(70);
        } else {
          setIsDeleting(false);
          setCurrentHeadlineIndex((prevIndex) => (prevIndex + 1) % headlines.length);
          // charIndex will be reset by the other branch
        }
      } else {
        if (charIndex < currentFullHeadline.length) {
          setDisplayedHeadline(currentFullHeadline.substring(0, charIndex + 1));
          setCharIndex(charIndex + 1);
          setTypingSpeed(120);
        } else {
          // Pause at the end of the headline
  ...
  }, [charIndex, isDeleting, typingSpeed, currentHeadlineIndex, headlines]);
